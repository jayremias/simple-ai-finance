...
function dateDiffDays(dateA: string, dateB: string): number {
  return Math.abs(new Date(dateA).getTime() - new Date(dateB).getTime()) / MS_PER_DAY;
}
...
export function matchTransactions(
  ...
  const matched: ParsedTransactionItem[] = [];
  const missing: ParsedTransactionItem[] = [];
  ...
  for (const importedItem of imported) {
    ...
    for (let existingIndex = 0; existingIndex < existing.length; existingIndex++) {
      ...
      if (isMatch(importedItem, existingItem)) {
        matched.push(importedItem);
        matchedExistingIndices.add(existingIndex);
        found = true;
        break;
      }
    }
    if (!found) missing.push(importedItem);
  }
...
export async function importStatement(
  ...
  const paddedFrom = new Date(new Date(earliest).getTime() - DATE_TOLERANCE_DAYS * MS_PER_DAY)
    .toISOString()
    .slice(0, 10);
  const paddedTo = new Date(new Date(latest).getTime() + DATE_TOLERANCE_DAYS * MS_PER_DAY)
    .toISOString()
    .slice(0, 10);
